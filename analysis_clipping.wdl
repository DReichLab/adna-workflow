	...
	command{
		...
		def clip_ends(bam, half_bases, minus_libraries, minus_bases, plus_libraries, plus_bases):
			...
			with open(clipped_bam + '.stderr', 'w') as clipping_stderr:
				subprocess.run(["java", "-Xmx3500m", "-jar", "${adna_screen_jar}", '${if hard_clip then "hardclip" else "softclip"}', "-b", "-n", "%d" % (half_bases,), "-i", bam, "-o", clipped_bam, "-x", "%d" % (minus_bases,), "-y", "%d" % (plus_bases,)] + minus_args + plus_args, stderr=clipping_stderr, check=True)
...
task haplogrep{
	...
	Int minimum_base_quality
	String region = "MT"
	Array[File] bams # bams should be clipped already
	File haplogrep_jar
	...
	command{
		...
		def haplogrep_run(bam):
			...
			subprocess.check_output("java -Xmx2600m -jar ${picard_jar} SortSam I=%s.realigned.bam O=%s.bam SORT_ORDER=coordinate" % (sample_id, sample_id), shell=True)
