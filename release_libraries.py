...
def add_read_groups(adna_jar_filename, demultiplexed_bam_filename, output_bam_filename, bam_date_string, label, library_id, individual, working_directory, jvm_mem_string):
	subprocess.run(["java", jvm_mem_string, "-jar", adna_jar_filename,
				 "AssignReadGroups", 
...
def build_release_library(adna_jar_filename, picard_jar, working_directory, library_parameters, jvm_mem_string):
	...
	with open('{}/stdout_build_release_library'.format(working_directory), 'w') as stdout_build, \
		...
		if len(library_component_bams) > 0: # merge any bams with reads and mark duplicates
			...
			subprocess.run("java {} -jar {} MergeSamFiles I={} O={} SORT_ORDER=coordinate".format(jvm_mem_string, picard_jar, ' I='.join(library_component_bams), library_with_duplicates_filename), shell=True, check=True, cwd=working_directory, stdout=stdout_build, stderr=stderr_build)
			...
			# deduplicate
			subprocess.run("java {0} -jar {1} MarkDuplicates I={2} O={3} M={3}.dedup_stats REMOVE_DUPLICATES=true BARCODE_TAG=XD ADD_PG_TAG_TO_READS=false MAX_FILE_HANDLES=1000".format(jvm_mem_string, picard_jar, to_deduplicate_filename, library_filename), shell=True, check=True, cwd=working_directory, stdout=stdout_build, stderr=stderr_build)
...
def process_library(adna_jar_filename, picard_jar, release_directory, library_parameters, jvm_mem_string):
	working_directory = library_parameters.library_id
	
	release_library_filename = build_release_library(adna_jar_filename, picard_jar, working_directory, library_parameters, jvm_mem_string)
...
if __name__ == "__main__":
	...
	results = [pool.apply_async(process_library, args=(adna_jar_filename, picard_jar, args.release_directory, parameters, jvm_mem_string)) for parameters in library_parameters]
	pool.close()
