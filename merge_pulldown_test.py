...
class TestMergePulldown(unittest.TestCase):
	def test_duplicate_ind_one_file(self):
		...
		with tempfile.TemporaryDirectory() as temp_directory:
			...
			self.assertRaisesRegex(ValueError, "individual duplicate appears more than once", merge_pulldown.merge_ind_files, output_filename, input_ind_filenames)
	...
	def test_duplicate_ind_two_files(self):
		...
		with tempfile.TemporaryDirectory() as temp_directory:
			...
			self.assertRaisesRegex(ValueError, "individual x appears more than once", merge_pulldown.merge_ind_files, output_filename, input_ind_filenames)
	...
	def test_combine(self):
		...
		with tempfile.TemporaryDirectory() as temp_directory:
			...
			merge_pulldown.merge_geno_files(output_filename, ['test/pulldown/x.geno', 'test/pulldown/yz.geno'], ind_counts, num_snps)
	...
	def test_bad_geno(self):
		...
		with tempfile.TemporaryDirectory() as temp_directory:
			output_filename = os.path.join(temp_directory, 'combined.geno')
			self.assertRaisesRegex(ValueError, "invalid genotype value\(s\)", merge_pulldown.merge_geno_files, output_filename, ['test/pulldown/bad.geno', 'test/pulldown/yz.geno'], ind_counts, num_snps)
	...
	def test_geno_too_many(self):
		...
		with tempfile.TemporaryDirectory() as temp_directory:
			output_filename = os.path.join(temp_directory, 'combined.geno')
			self.assertRaisesRegex(ValueError, "extra values in", merge_pulldown.merge_geno_files, output_filename, ['test/pulldown/too_many.geno', 'test/pulldown/yz.geno'], ind_counts, num_snps)
